...
import { Filter, Download, Calendar, ArrowDownRight, AlertCircle, CheckCircle2, Clock, ChevronLeft, ChevronRight, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react"
...
export default function DespesasPage() {
    ...
    const despesaCategories = categories.filter((c: { tipo: string }) => c.tipo === 'despesa')
    ...
    const getStatusBadge = (status: string) => {
        const statusConfig = {
            liquidado: { className: "bg-emerald-500/20 text-emerald-600 hover:bg-emerald-500/30", label: "Pago" },
            pendente: { className: "bg-amber-500/20 text-amber-600 hover:bg-amber-500/30", label: "Pendente" },
        ...
        <div className="space-y-8 max-w-[1600px] mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Controle suas saídas, custos fixos e variáveis</p>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
                    ...
                    <TransactionForm type="despesa" onSuccess={refresh} />
                </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-4">
                    ...
                    <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] text-muted-foreground">Comprometimento total</p>
                            </div>
                    ...
                    <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] text-muted-foreground">Liquidado com sucesso</p>
                            </div>
                    ...
                    <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] text-muted-foreground">Vencendo nos próximos dias</p>
                            </div>
                ...
                <Card className="border-none bg-zinc-900/5 dark:bg-zinc-100/10 shadow-none overflow-hidden group hover:shadow-md transition-all border-l-4 border-l-rose-600">
                    <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                            <div className="space-y-1">
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Atrasado</p>
                                {statsLoading ? (
                                    <Skeleton className="h-9 w-32" />
                                ) : (
                                    <h3 className="text-3xl font-bold text-rose-700 dark:text-rose-400 tracking-tight">
                                        {formatCurrency(stats.atrasado)}
                                    </h3>
                                )}
                                <p className="text-[10px] text-muted-foreground font-bold">Atenção! Necessário pagar</p>
                            </div>
                            <div className="h-10 w-10 rounded-xl bg-rose-700/10 flex items-center justify-center font-bold">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Select value={statusFilter} onValueChange={(value: any) => { setStatusFilter(value); setCurrentPage(1); }}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="liquidado">Pago</SelectItem>
                            ...
                            <Select value={categoriaFilter} onValueChange={(value) => { setCategoriaFilter(value); setCurrentPage(1); }}>
                                ...
                                <SelectContent>
                                    ...
                                    {despesaCategories.map((cat: { id: string; nome: string }) => (
                        ...
                        <TableHeader>
                            <TableRow className="border-none hover:bg-transparent text-muted-foreground">
                                ...
                                <TableHead className="font-medium text-xs h-10 cursor-pointer hover:text-foreground transition-colors" onClick={() => toggleSort('data_vencimento')}>
                                    <div className="flex items-center">Vencimento {getSortIcon('data_vencimento')}</div>
                                </TableHead>
                            ...
                            {loading ? (
                                ...
                                transactions.map((transaction) => (
                                    <TableRow key={transaction.id} className="border-neux-1/5 hover:bg-zinc-900/5 dark:hover:bg-zinc-100/5 transition-colors">
                                        ...
                                        <TableCell className="text-right font-bold text-rose-500 px-6">
